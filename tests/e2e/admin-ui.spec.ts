...
test.describe('Admin UI', () => {
  test('liste les soumissions et affiche les actions', async ({ page }) => {
    await page.route('**/admin/spots/pending**', async route => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({ items: [
          { spotId: 's1', createdAt: '2025-01-01T10:00:00Z', status: 'pending', name: 'Spot 1', type: 'ponton', submittedBy: 'alice' },
          { spotId: 's2', createdAt: '2025-01-02T12:00:00Z', status: 'pending', name: 'Spot 2', type: 'association', submittedBy: 'bob' }
        ] })
      });
    });
  ...
  // Attendre le tableau
    await expect(page.getByText('Modération — Soumissions en attente')).toBeVisible();
  // Attendre que les données soient chargées et vérifier les champs saisis
  await expect(page.getByPlaceholder('Note de modération').first()).toBeVisible();
  ...
  await expect(page.getByText('Refuser')).toHaveCount(2);
    ...
    const note = page.getByPlaceholder('Note de modération').first();
    await note.fill('RAS');
      await Promise.all([
